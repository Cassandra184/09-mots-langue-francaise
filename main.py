"""
    Exercice sur les mots de la langue française
"""
...
def read_data(filename):
    ...
    'gloire'
    """
    with open(filename, encoding="utf-8") as f: return [line.strip() for line in f]
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    return{
        ...
        and any(mot.startswith(p) for p in lstart)
        and any(m in mot for m in lmid)
        and any(mot.endswith(s) for s in lstop)
    }


def main() :
    """
    main() permet de tester les différentes méthodes de l'exercice 
    """
    mots = read_data(FILENAME)
